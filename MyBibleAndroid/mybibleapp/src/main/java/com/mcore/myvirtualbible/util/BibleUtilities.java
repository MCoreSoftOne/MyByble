...
public class BibleUtilities {
	...
	public static boolean isReadWriteStoragePermissionGranted(Context ctx) {
	...
	public static boolean verifyReadWriteStoragePermission(Context ctx, int requestCode) {
		boolean result = ContextCompat.checkSelfPermission(ctx,
				Manifest.permission.WRITE_EXTERNAL_STORAGE)
				!= PackageManager.PERMISSION_GRANTED || ContextCompat.checkSelfPermission(ctx,
				Manifest.permission.READ_EXTERNAL_STORAGE)
				!= PackageManager.PERMISSION_GRANTED;
		if (result && ctx instanceof Activity) {
